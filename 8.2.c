...
int printArgumentInTheMiddle(void* argument, enum ArgumentType type, unsigned width)
{
    ...
    {
        printCharInTheMiddle(argument, width);
    }
    else if(type == CHARS)
    {
        printCharsInTheMiddle(argument, width);
    }
    else if(type == INT)
    {
        printIntInTheMiddle(argument, width);
    }
    else if(type == DOUBLE)
    {
        printDoubleInTheMiddle(argument, width);
    }
}

void printCharInTheMiddle(void* argument, unsigned width)
{
    const char znak = *(char*)argument;
    if(isprint(znak) == 0)
    {
        printf("\n");
    }
...
void printCharsInTheMiddle(void* argument, unsigned width)
{
    const char* napis = *(char**)argument;
    const int dl_napisu = strlen(napis);
    if(width<dl_napisu)
    {
        printf("%s\n", napis);
    }
...
void printDoubleInTheMiddle(void* argument, unsigned width)
{
    const double liczba = *(double*)argument;
...
void printIntInTheMiddle(void* argument, unsigned width)
{
    const int liczba = *(int*)argument;
